...
Q.1.1.8. APGAR [concept: Apgar, field_type: number, min: 0, tt_onLoad: showApgarControl(1); __$("keyboard").style.display = "none", max: 10, pos: 7, tt_pageStyleClass: NoKeyboard, absoluteMin: 0, absoluteMax: 10, helpText: 1<sup>st</sup> Minute APGAR <%= params["baby"] + ((params["baby"].to_i == 1) ? "<sup>st</sup>" | ((params["baby"].to_i == 2) ? "<sup>nd</sup>" | ((params["baby"].to_i == 3) ? "<sup>rd</sup>" | "<sup>th</sup>" ))) %> baby]

Q.1.1.9. APGAR [concept: Apgar, field_type: number, min: 0, tt_onLoad: showApgarControl(5); __$("keyboard").style.display = "none", max: 11, pos: 8, tt_pageStyleClass: NoKeyboard, absoluteMin: 0, absoluteMax: 10, helpText: 5<sup>th</sup> Minute APGAR <%= params["baby"] + ((params["baby"].to_i == 1) ? "<sup>st</sup>" | ((params["baby"].to_i == 2) ? "<sup>nd</sup>" | ((params["baby"].to_i == 3) ? "<sup>rd</sup>" | "<sup>th</sup>" ))) %> baby]

Q.1.1.10. Delivery Mode [pos: 9, name: concept[Delivery Mode], ajaxURL: /encounters/concept_set_options?set=delivery_mode&search_string=, field_type: text, concept: DELIVERY MODE, helpText: Delivery Mode <%= params["baby"] + ((params["baby"].to_i == 1) ? "<sup>st</sup>" | ((params["baby"].to_i == 2) ? "<sup>nd</sup>" | ((params["baby"].to_i == 3) ? "<sup>rd</sup>" | "<sup>th</sup>" ))) %> baby]

Q.1.1.11. Next URL [pos: 10, name: next_url, value: /protocol_patients/bba_delivery?user_id=<%= @user.id %>&patient_id=<%= @patient.id %>&baby=<%= params["baby"].to_i + 1 %>&baby_total=<%= params["baby_total"]%>, type: hidden, ruby: <%= (params["baby"].to_i >= params["baby_total"].to_i ? "disabled" | "") %>]

Q.1.1.12. Next URL [pos: 11, name: next_url, value: /protocol_patients/mother_delivery_details?user_id=<%= @user.id %>&patient_id=<%= @patient.id %>, type: hidden, ruby: <%= (params["baby"].to_i < params["baby_total"].to_i ? "disabled" | "") %>]

Q.1.1.13. Baby Check [pos: 12, name: baby_check, type: hidden, value: true]
