...
BEGIN
	...
	SET @outcometype = (SELECT name FROM encounter_type WHERE encounter_type_id = (SELECT encounter_type FROM encounter WHERE encounter_id = new.encounter_id));
  	...
  	IF new.concept_id = (SELECT concept_id FROM concept_name WHERE name = "BABY OUTCOME" LIMIT 1) AND @type != "CURRENT BBA DELIVERY" AND @outcometype = "BABY DELIVERY" THEN
	...
	IF new.concept_id = (SELECT concept_id FROM concept_name WHERE name = "DELIVERY MODE" LIMIT 1) AND @type != "CURRENT BBA DELIVERY" AND @outcometype = "BABY DELIVERY" THEN
	...
	IF new.concept_id = (SELECT concept_id FROM concept_name WHERE name = "DIAGNOSIS" LIMIT 1) THEN
		SET @diagnosis = (SELECT name FROM concept_name WHERE concept_id = new.value_coded LIMIT 1);	
