class Observation < ActiveRecord::Base
  ...
  def answer_string(tags=[])
    ...
    coded_name = "#{coded_answer_name} #{self.value_modifier}#{self.value_text} #{self.value_numeric}#{(!formatted_name.nil? ? (formatted_name.downcase.match(/time/) ? self.value_datetime.to_time.strftime("%H:%M") : self.value_datetime.strftime("%d/%b/%Y")) : self.value_datetime.strftime("%d/%b/%Y")) rescue nil}#{self.value_boolean && (self.value_boolean == true ? 'Yes' : 'No' rescue nil)}#{' ['+order.to_s+']' if order_id && tags.include?('order')}"
