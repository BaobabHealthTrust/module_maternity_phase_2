...
EOF
  ...
  def label
    ...
    when "UPDATE OUTCOME"
      if self.to_s.upcase.include?("DISCHARGED")
        ...
        label.draw_multi_text("DISCHARGED BY: #{User.find(self.provider).name rescue ''}")
        label.print
      end
    end
  end
  ...
  def discharge_summary
    [self.observations.collect{|ob|
      ob.answer_string if (ob.concept.name.name.upcase.strip == "DIAGNOSIS" rescue false)
    }.compact,
...
def discharge_date
  ...
  Encounter.find(:first, :order => ["encounter_datetime ORDER date_created DESC"], :joins => [:observations],
      :conditions => ["encounter.encounter_type = ? AND obs.concept_id = ? AND encounter.patient_id = ?",
      EncounterType.find_by_name("UPDATE OUTCOME").id, ConceptName.find_by_name("DISCHARGED").concept_id, self.patient_id]).encounter_datetime.to_date rescue nil
